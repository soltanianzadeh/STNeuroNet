...
function ViewNeurons(img,Mask,Trace)
...
%                 text(gui.TraceAxes,data1.lk+10,data1.pk,num2str((1:numel(data1.pk))'));
            ...
            ylabel(gui.TraceAxes,'F or df/f (%)');
            set(gui.TraceAxes,...
    ...
    %-------------------------------------------------------------------------%
        function PushNextButton(~, ~ )
            previous = i;
            i = min(previous +1,numel(resultString));
            if IsPlaying
                CallBackInterrupted = 1;
            end
            
            if (i< numel(resultString))
                updateInterface();
            else
                close(gui.Window);
            end
